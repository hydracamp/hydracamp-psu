...
require 'spec_helper'
require 'hydra/generic_file'

describe Hydra::GenericFile do
  before(:each) do
    Hydra::GenericFile.any_instance.stub(:terms_of_service).and_return('1')
    @file = Hydra::GenericFile.new
    @file.apply_depositor_metadata('jcoyne')
  end
  describe "attributes" do
    it "should have rightsMetadata" do
      @file.rightsMetadata.should be_instance_of Hydra::ParanoidRightsDatastream
    end
    it "should have properties datastream for depositor" do
      @file.properties.should be_instance_of Hydra::PropertiesDatastream
    end
    ...
    it "should have a dc desc metadata" do
      @file.descMetadata.should be_kind_of Hydra::GenericFileRdfDatastream
    end
    ...
    if defined? Resque
      describe "that have been saved" do
        ...
        it "should have activity stream-related methods defined" do
          @file.save
          f = Hydra::GenericFile.find(@file.pid)
  ...
  describe "audit" do
    before(:all) do
      Hydra::GenericFile.any_instance.stub(:terms_of_service).and_return('1')
      Hydra::GenericFile.any_instance.stub(:characterize).and_return(true)
      f = Hydra::GenericFile.new
      f.add_file_datastream(File.new(Rails.root + 'spec/fixtures/world.png'), :dsid=>'content')
      f.apply_depositor_metadata('mjg36')
      f.save
      @f = Hydra::GenericFile.find(f.pid)
      @datastreams = [FileContentDatastream, ActiveFedora::RelsExtDatastream,
                      ActiveFedora::Datastream, Hydra::PropertiesDatastream,
                      Hydra::ParanoidRightsDatastream]
    end
    after(:all) do
      @f.delete
    end
    if defined? Resque
      it "should schedule a audit job" do
        @datastreams.each { |ds| ds.any_instance.stub(:dsChecksumValid).and_return(false) }
        ChecksumAuditLog.stub(:create!).and_return(true)
    ...
    it "should log a passing audit" do
      @datastreams.each { |ds| ds.any_instance.stub(:dsChecksumValid).and_return(true) }
      @f.audit!
  ...
  describe "related_files" do
    ...
    before(:each) do
      @f1 = Hydra::GenericFile.new(:pid => "foobar:1")
      @f2 = Hydra::GenericFile.new(:pid => "foobar:2")
      @f3 = Hydra::GenericFile.new(:pid => "foobar:3")
    ...
    it "should work when batch is not defined by querying solr" do
      ...
      @f1.save
      @f2.save
      @f1.should_receive(:batch).twice.raises(NoMethodError)
  ...
  #   before(:all) do
  #     Hydra::GenericFile.any_instance.stub(:terms_of_service).and_return('1')
  #     Hydra::GenericFile.any_instance.should_receive(:characterize_if_changed)#.yields
  #     @new_file = Hydra::GenericFile.new(:pid => 'ns:123')
  ...
  describe "characterize" do
    ...
    describe "after job runs" do
      before(:all) do
        ...
        myfile.save
        @myfile = Hydra::GenericFile.find(myfile.pid)
      end
      after(:all) do
